...
@Component({
  selector: 'vex-offer-progress-dialog',
  templateUrl: 'offer-progress-dialog.component.html',
})
export class OfferProgressDialogComponent {
  constructor(
    private dialogRef: MatDialogRef<OfferProgressComponent>,
    @Inject(MAT_DIALOG_DATA) public data: { result: boolean }
  ) {}
  close() {
...
export class OfferProgressComponent implements OnInit, AfterViewChecked {
  ...
  ngAfterViewChecked(): void {
    ...
    const slabels = Array.from(
      document.querySelectorAll('.mat-step-header .mat-step-label')
    );
    ...
    if (document.querySelectorAll('.mat-step-header').length > 0) {
      if (this.packinfo) {
        document
          .querySelectorAll('.mat-step-header')[0]
          .classList.add('mat-step-label-active');
      } else {
        if (!this.isAdvertiser) {
  ...
  savePackingInfo(stepper: MatStepper) {
    ...
    this.offer.ship_date = datepipe.transform(
      new Date(this.packageform.value.shipDate),
      'MM.dd.yyyy'
    );
    this.offer.arrv_date = datepipe.transform(
      new Date(this.packageform.value.arrivalDate),
      'MM.dd.yyyy'
    );

    console.log(
      'save package1 ',
      this.packageform.value.shipDate,
      this.offer.track_no,
      this.offer.ship_carrier,
      this.packageform.value.arrivalDate,
      this.offer
    );
    ...
    if (this.offer.offer_progress != null) {
      this.dataService
        .savePackageDetail(
          this.offer.offer_progress.package.package_uuid,
          formData
        )
        .pipe()
        .subscribe((cdata: any) => {
  ...
  saveShareLink(stepper: MatStepper) {
    ...
    if (this.offer.offer_progress != null) {
      this.dataService
        .provideUploadedContent(
          this.offer.offer_progress.uploaded_content.upload_uuid,
          formData
        )
        .pipe()
        .subscribe((cdata: any) => {
